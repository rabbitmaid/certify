<x-app-layout pageTitle="Internship Batches">
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('All Internship Batches') }}
            </h2>

            <x-link-button href="{{ route('internship-batch.create') }}">Add New </x-link-button>
        </div>
    </x-slot>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="flex items-center justify-end gap-x-2">
                            ...
                            <a href="{{ route('internship-session.show', $internshipBatch->id) }}" class="bg-gray-800 text-white py-1 px-2 rounded text-center text-sm hover:underline hover:opacity-95 hover:shadow-sm" title="View">
                            ...
                            <a href="{{ route('internship-session.edit', $internshipBatch->id) }}" class="bg-yellow-500 py-1 px-2 rounded text-center text-sm hover:underline hover:opacity-95 hover:shadow-sm" title="Edit">
                                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M16.7574 2.99678L14.7574 4.99678H5V18.9968H19V9.23943L21 7.23943V19.9968C21 20.5491 20.5523 20.9968 20 20.9968H4C3.44772 20.9968 3 20.5491 3 19.9968V3.99678C3 3.4445 3.44772 2.99678 4 2.99678H16.7574ZM20.4853 2.09729L21.8995 3.5115L12.7071 12.7039L11.2954 12.7064L11.2929 11.2897L20.4853 2.09729Z"></path>
                                </svg>
                            </a>

                            <form id="delete-form-{{ $internshipBatch->id }}" action="{{ route('internship-session.destroy') }}" method="POST" class="delete-form inline">
                                @csrf
                                @method('DELETE')

                                <input type="hidden" name="id" value="{{ $internshipBatch->id }}">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="text-center text-gray-500 py-4">No Internship Batch found.</td>
                    </tr>
                    @endforelse
