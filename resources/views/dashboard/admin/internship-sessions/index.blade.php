<x-app-layout pageTitle="Internship Sessions">
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('All Internship Sessions') }}
            </h2>

            <x-link-button href="{{ route('internship-session.create') }}">Add New </x-link-button>
        </div>
    </x-slot>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="flex items-center justify-end gap-x-2">
                            ...
                            <a href="{{ route('internship-session.show', $internshipSession->id) }}" class="bg-gray-800 text-white py-1 px-2 rounded text-center text-sm hover:underline hover:opacity-95 hover:shadow-sm" title="View">
                            ...
                            <a href="{{ route('internship-session.edit', $internshipSession->id) }}" class="bg-yellow-500 py-1 px-2 rounded text-center text-sm hover:underline hover:opacity-95 hover:shadow-sm" title="Edit">
                                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                            ...
                            <form id="delete-form-{{ $internshipSession->id }}" action="{{ route('internship-session.destroy') }}" method="POST" class="delete-form inline">
                                ...
                                <input type="hidden" name="id" value="{{ $internshipSession->id }}">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="text-center text-gray-500 py-4">No Internship Sessions found.</td>
                    </tr>
                    @endforelse
